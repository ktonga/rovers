...
object Parser {
  ...
  def parsePosition(upperRight: Coordinate, occupied: List[Coordinate], s: String): MaybeSuccess[Position] = {
    val xs = s.split(" ").toList
    xs match {
      case x :: y :: d :: Nil =>
        for {
          ...
          _ <- if (occupied contains coord) -\/(s"($x, $y) is occupied by other rover already")
               else \/-(())
